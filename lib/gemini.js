...
const DETECT_MODEL = process.env.GEMINI_DETECT_MODEL || 'gemini-3.5-flash-lite';
...
async function call(model, parts, temperature = 0) {
  ...
  // Gemini 3.x models default to heavy "thinking" which adds 20-30s latency;
  // low keeps quality fine for extraction/matching while staying demo-fast.
  if (/gemini-3\./.test(model) && !/lite/.test(model)) {
