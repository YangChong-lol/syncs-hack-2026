...
async function composeInvite(fromUser, toUser, dishes) {
  if (isMock()) {
    ...
    return {
      subject: `A neighbour wants to cook ${dishes[0]?.name || 'something'} with you`,
      body:
        `Hi ${toUser.name},\n\n` +
        `A neighbour on SaveMyFridge (about ${Math.max(1, Math.round((toUser.distance_m || 500) / 100) / 10)} km away) scanned their fridge, ` +
        ...
        `Reply through SaveMyFridge to accept - neither of you sees the other's email until you both agree.\n\n` +
        `Happy cooking,\nSaveMyFridge (demo mode)`,
      mock: true,
    };
  }
  const input = {
